         ...
         {active !== null && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={transition}>
               {active === item && (
                  ...
                  >
                     <motion.div transition={transition} layoutId="active" className={`relative ${children && 'bg-coffee-dark rounded-[10px] border border-black/20 shadow-xl'}`}>
...
export const Menu = ({ setActive, children }) => {
   ...
   return (
      <MenuHoverContext.Provider value={{ registerHover, unregisterHover }}>
         <article onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className="relative rounded-full border border-transparent shadow-input flex justify-center py-4 xl:py-6">
            {children}
         </article>
...
export const HoveredLink = ({ children, className, ...rest }) => {
   return (
      <Link prefetch={true} {...rest} className={cn('px-4 py-2.5', className)}>
         {children}
      </Link>
   );
};

export const NestedHoveredLink = ({ children, className, submenu, ...rest }) => {
   ...
   return (
      ...
      >
         ...
         {submenu && showSubmenu && (
            ...
            >
               <div className="flex flex-col">{submenu}</div>
            </motion.div>
         )}
      </div>
   );
