...
const titleItemsData = [
   {
      ...
      content: {
         ...
         items: [
            {
               type: 'paragraph',
               text: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Quia deserunt odit id adipisci cumque molestias fuga ratione ea fugit temporibus. Iusto illo odio esse ipsam omnis, adipisci fugiat minus ullam?',
            },
            {
               type: 'paragraph',
               text: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Quia deserunt odit id adipisci cumque molestias fuga ratione ea fugit temporibus. Iusto illo odio esse ipsam omnis, adipisci fugiat minus ullam?',
...
export default function TitleImagesDeney2() {
   ...
   const displayItems = selectedItem ? titleItemsData.filter((item) => item.value === selectedItem) : titleItemsData;
                     ...
                     <AccordionItem value={item.value} className="relative">
                        ...
                        <AccordionContent className="relative flex justify-end px-0 py-0 data-[state=open]:overflow-auto pb-0 md:pb-2 z-20">
                           <section className="w-full md:w-2/3 -mt-28 md:-mt-32.5 flex flex-col items-start gap-3 lg:gap-3">
                              ...
                              >
                                 <Image src={item.image} alt={item.title} fill className="object-cover object-top rounded-sm" />
                              </motion.div>
                              ...
                              <main className="w-full py-3">
                                 <article className="w-full flex flex-col items-start">
                                    ...
                                    {item.content.items.map((contentItem, idx) => (
                                       <div key={idx}>
                                          {contentItem.type === 'paragraph' && <p className="text-ivory-soft/80 text-sm mb-3">{contentItem.text}</p>}
