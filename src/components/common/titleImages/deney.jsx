...
const titleItemsData = [
    {
        ...
        content: {
            ...
            items: [
                { type: 'list', items: ['FUE Micro Motor', 'FUE Manuel', 'Body Hair Transplant (BHT)', 'Long Hair Transplant', 'FUT', 'Reconstructive Hair Transplant'] },
            ]
        }
    },
    {
        ...
        content: {
            ...
            items: [
                { type: 'list', items: ['FUE Micromotor', 'FUE Manuel', 'BHT', 'Reconstructive Hair Transplant'] },
            ]
        }
    },
    {
...
export default function DeneyTitleImage() {
    ...
    return (
        <MotionScrollInView
            ...
            <Accordion type="single" collapsible className="w-full flex flex-col md:flex-row min-h-[70vh] md:min-h-min md:h-150 items-stretch gap-0" value={selectedItem} onValueChange={(value) => setSelectedItem(value)}>
                {titleItemsData.map((item) => {
                    const isOpen = selectedItem === item.value;
                    return (
                        <div
                            key={item.value}
                            className={`relative flex-1 w-full md:w-auto h-full transition-all duration-400 ease-in-out flex ${isOpen ? 'md:flex-7 h-125' : 'md:flex-1 h-auto'}`}
                        >
                            <AccordionItem value={item.value} className={cn('flex-1 border-none')}>
                                <div className="relative h-full flex-1 flex flex-col gap-0 w-full overflow-hidden">
                                    ...
                                    >
                                        ...
                                        {isOpen && (
                                            <div className="md:hidden w-full h-min bg-coffee-dark/60 flex flex-row md:flex-col items-center justify-center md:justify-between py-6 cursor-pointer">
                                                <div className="mx-auto w-24 h-1.5 rounded-sm bg-gold/30" />
                                            </div>
                                        )}
                                    </AccordionTrigger>
